;(function ($) {
  ...
  class GSAPAnimation {
    static Init() {
      ...
      $('.right-separetor').length &&
        this.width100ScrollTriggerAnimation('.right-separetor')
      $('.left-separetor').length &&
        this.width100ScrollTriggerAnimation('.left-separetor')
      $('.title-animation').length &&
        this.sectionTitleAnimation('.title-animation')
    }
    ...
    static sectionTitleAnimation(activeClass) {
      ...
      sectionTitleLines.forEach((sectionTextLine) => {
        ...
        const itemSplitted = new SplitText(sectionTextLine, {
          type: 'chars, words',
        })
    ...
    static upDownScrollTriggerAnimation(titleClass) {
      ...
      Title.forEach((TitleSingle) => {
        ...
        const TitleSingleTargetHeight =
          TitleSingle.getAttribute('data-target-height')
    ...
    if ($(item).data('preloader') === 'active') {
      if ($(item).data('loaded') === 'progress') {
        ...
        $('.preloader-close').on('click', () =>
          RRDevsInit.LoadedAfterProgress('click'),
        )
  ...
  const scrollTopPercentage = () => {
    const scrollPercentage = () => {
      ...
      scrollElementWrap.css(
        'background',
        `conic-gradient( var(--rr-theme-primary) ${scrollValue}%, var(--rr-heading-primary) ${scrollValue}%)`,
      )
  ...
  /*======================================
	...
	========================================*/
  var link = $(
    '.onepagenav #mobile-menu ul li a, .onepagenav .mean-nav ul li a',
  )
  ...
  $(
    '.live-comment-widget__form-input-select select, .contact-us-message__form-input-select select, .get-in-touch__form-input-select select, .banner-3__form-input-select select, .contact__form-input-select select',
  ).niceSelect()
    ...
    magnetize(e) {
      ...
      this.settings.target.each((index, element) => {
        let $this = $(element),
          ...
          attraction =
            $this.data('magnetic-attraction') || this.settings.attraction,
  ...
  let portfolioDetails__media__slider = new Swiper(
    '.project-details__media__slider',
    {
      slidesPerView: '1',
      centeredSlides: true,
      loop: true,
